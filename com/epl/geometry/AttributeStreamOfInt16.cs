...
namespace com.epl.geometry
{
	internal sealed class AttributeStreamOfInt16 : com.epl.geometry.AttributeStreamBase
	{
		...
		public AttributeStreamOfInt16(com.epl.geometry.AttributeStreamOfInt16 other)
		{
			m_buffer = (short[])other.m_buffer.Clone();
			m_size = other.m_size;
		}

		public AttributeStreamOfInt16(com.epl.geometry.AttributeStreamOfInt16 other, int maxSize)
		{
			m_size = other.Size();
			if (m_size > maxSize)
			{
		...
		{
			return com.epl.geometry.VertexDescription.Persistence.enumInt16;
		}
		...
		public override bool Equals(com.epl.geometry.AttributeStreamBase other, int start, int end)
		{
			...
			if (!(other is com.epl.geometry.AttributeStreamOfInt16))
			{
				return false;
			}
			com.epl.geometry.AttributeStreamOfInt16 _other = (com.epl.geometry.AttributeStreamOfInt16)other;
			int size = Size();
		...
		public override void AddRange(com.epl.geometry.AttributeStreamBase src, int start, int count, bool bForward, int stride)
		{
			...
			else
			{
				...
				for (int i = 0; i < count; i += stride)
				{
					...
					{
						m_buffer[oldSize + i + s] = ((com.epl.geometry.AttributeStreamOfInt16)src).m_buffer[start + n + s];
					}
				}
			}
		}
		...
		public override void InsertRange(int start, com.epl.geometry.AttributeStreamBase src, int srcStart, int count, bool bForward, int stride, int validSize)
		{
			...
			if (m_buffer == ((com.epl.geometry.AttributeStreamOfInt16)src).m_buffer)
			{
				if (start < srcStart)
				{
					srcStart += count;
			...
			if (bForward)
			{
				System.Array.Copy(((com.epl.geometry.AttributeStreamOfInt16)src).m_buffer, srcStart, m_buffer, start, count);
			}
			else
			{
				int n = count;
				for (int i = 0; i < count; i += stride)
				{
					...
					for (int s = 0; s < stride; s++)
					{
						m_buffer[start + i + s] = ((com.epl.geometry.AttributeStreamOfInt16)src).m_buffer[srcStart + n + s];
					}
				}
			}
		}
		...
		public override void InsertAttributes(int start, com.epl.geometry.Point pt, int semantics, int validSize)
		{
			...
			{
				m_buffer[start + c] = (short)pt.GetAttributeAsDbl(semantics, c);
			}
		}
		...
		public override void WriteRange(int startElement, int count, com.epl.geometry.AttributeStreamBase _src, int srcStart, bool bForward, int stride)
		{
			...
			com.epl.geometry.AttributeStreamOfInt16 src = (com.epl.geometry.AttributeStreamOfInt16)_src;
			// the input
			// type must
			// match
