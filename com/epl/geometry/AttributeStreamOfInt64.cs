...
{
	internal sealed class AttributeStreamOfInt64 : com.epl.geometry.AttributeStreamBase
	{
		...
		public override void AddRange(com.epl.geometry.AttributeStreamBase src, int start, int count, bool bForward, int stride)
		{
			...
			if (bForward)
			{
				System.Array.Copy(((com.epl.geometry.AttributeStreamOfInt64)src).m_buffer, start, m_buffer, oldSize, count);
			}
			else
			{
				int n = count;
				for (int i = 0; i < count; i += stride)
				{
					...
					{
						m_buffer[oldSize + i + s] = ((com.epl.geometry.AttributeStreamOfInt64)src).m_buffer[start + n + s];
					}
				}
			}
		}
		...
		public override void InsertRange(int start, com.epl.geometry.AttributeStreamBase src, int srcStart, int count, bool bForward, int stride, int validSize)
		{
			...
			if (m_buffer == ((com.epl.geometry.AttributeStreamOfInt64)src).m_buffer)
			{
				if (start < srcStart)
				{
					srcStart += count;
			...
			if (bForward)
			{
				System.Array.Copy(((com.epl.geometry.AttributeStreamOfInt64)src).m_buffer, srcStart, m_buffer, start, count);
			}
			else
			{
				int n = count;
				for (int i = 0; i < count; i += stride)
				{
					...
					for (int s = 0; s < stride; s++)
					{
						m_buffer[start + i + s] = ((com.epl.geometry.AttributeStreamOfInt64)src).m_buffer[srcStart + n + s];
					}
				}
			}
		}
